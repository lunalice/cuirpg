...
public class Hero extends Character implements BattleInf {
	private final static double growth = 1.1; // キャラ成長倍率
	...
	public void levelUp() {
		...
		if (nextLevel > level) {
			...
			this.maxHP = maxHP * (int)growth;
			this.hp = maxHP;
			this.maxMp = maxMp * (int)growth;
			this.mp = maxMp;
			this.attackPower = attackPower * (int)growth;
			this.defencePower = defencePower * (int)growth;
		}
	}
